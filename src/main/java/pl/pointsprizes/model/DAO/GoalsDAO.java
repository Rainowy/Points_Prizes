...
public class GoalsDAO {
    ...
    public static void addToDb(Goals goal) {
        ...
        params[3] = String.valueOf(User.getCurrentUser().getId());    //pobiera Id aktualnego użytkownika i wstawia do kolumny user_id
    ...
    /**
     * Pobiera wszystkie obiekty goal konkretnego użytkownika
     **/
    public static List<Goals> getCurrentUserGoals() {

        List<Goals> goalsList = new ArrayList<>();

        for (String[] s : getAllFromGoalsBasedOnUserId(User.getCurrentUser().getId())) {
            Goals goal = getGoal(s);    // odwołuje się do metody getGoal która zwraca pojedynczy obiekt Goal
            goalsList.add(goal);
        }
        return goalsList;
